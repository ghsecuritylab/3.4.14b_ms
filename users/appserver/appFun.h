...
#define APP_DEBUG_ON_TAG_FILE	"/var/run/APP_DEBUG"

#define APP_DEBUG(fmt, args...)	 \
	{											\
		if(access( APP_DEBUG_ON_TAG_FILE, F_OK ) >= 0) \
		{										\
			printf("[Debug]:"fmt"\n", ##args); \
		}										\
	}
...
extern int get_child_mesh_name(char *p_buffer, char *p_mac, char *p_name);
extern int set_child_mesh_name(char *p_mac, char *p_name);
extern int str_to_mac_addr(char *buf, char *mac);
extern int reset_child_mesh_name(  char *p_mac, char *p_name);
#endif