            ...
            if (FD_ISSET( sd , &readfds)) 
            {
				...
				*/
                ...
                else
                {
					...
					{
						while(NULL != (p_stick = strstr(p_tmp+4, "POST")))
						{
							/*address the event from app client*/
							strncpy(packet, p_tmp, p_stick-p_tmp);
							APP_DEBUG("stick packet[%d] == %s\n", size, packet);
							app_event_handler(sd, packet);
							p_tmp = p_stick;
							size++;
						}
						APP_DEBUG("stick packet[%d] = [%s]\n", size, p_tmp);
						app_event_handler(sd, p_tmp);
					} 					
                }
