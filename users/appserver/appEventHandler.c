...
#include "appUpload.h"
#include "../boa/src/slaveUpgrade.h"
#include "../boa/src/remoteUpgrade.h"
...
int get_app_request_type(const char* sourceStr)
{
	...
	APP_DEBUG("[appserver][%s-%d]source targetStr = %s strlen(targetStr) = %d\n",
...
{	
	...
	for (; i < iCount; ++i) 
	{
		...
		if (!strcmp(upgrade, "on"))
		{
			...
			remoteChekUpgrade();
			if (1 ==  remoteUpgradeInfo.checkVersionStatus)
			{
			...
			{
				printf("upgrade error:checkVersionStatus is not 1 !!!! \n");
				goto retError;
			}
		...
		{  
			...
			if (!strcmp(upgrade, "on"))
			{
				...
				if (1 == get_slave_ip_from_device_name(childName, slave_ip))
				{
					printf("slave_ip = %s\n", slave_ip);
					ret = SlaveStartRemoteUpgrade(slave_ip, 80);
					printf("SlaveStartRemoteUpgrade ret = %d\n", ret);
					if (1 == ret)
					{
...
{	
	...
	for (; i < iCount; ++i) {  
		...
		*/
		if ( MAC == NULL || IP == NULL || location == NULL)
		{
			goto retError;
		}
		...
		if (strlen(location))
			reset_child_mesh_name(MAC, location);
		DTRACE(DTRACE_APPSERVER, "IP = %s\n", IP);
		DTRACE(DTRACE_APPSERVER, "MAC = %s\n", MAC);
		//DTRACE(DTRACE_APPSERVER,"removeDev = %s\n", removeDev);
...
#if 0
{
	...
	if (env_sta_info.sta_num > 80)
		show_num = 80;
	else
		show_num = env_sta_info.sta_num;

	sprintf(buffer, "%d", show_num);
...
int send_upgrade_conf_to_client(int fd)
{
	...
	for(i = 0; i < childNum; i++)
	{
		...
		cJSON_AddItemToArray(array, arrayobj = cJSON_CreateObject());
		memset(buffer, 0, sizeof(buffer));
		//get_child_mesh_name(buffer, childNum,mesh_json_s.main_info.neighbor_info[i].mac, mesh_json_s.main_info.neighbor_info[i].name);
		cJSON_AddStringToObject(arrayobj, "childName", mesh_json_s.main_info.neighbor_info[i].name);
		memset(buffer, 0, sizeof(buffer));
		//apmib_get(MIB_KLINK_SLAVE1_SOFT_VERSION+i, (void *)buffer);
		
		char targetInfo[32] = {0};
		memset(buffer, 0, sizeof(buffer));
		str_to_mac_addr(buffer, mesh_json_s.main_info.neighbor_info[i].mac);
		printf("[app] getSlaveVersion  mac=%s", buffer);
		getSlaveVersion(targetInfo, buffer, 0);
		
		printf("[app] version %s\n", targetInfo);
		cJSON_AddStringToObject(arrayobj, "childLocalVersion", targetInfo);
...
int send_linkdev_conf_to_client(int fd)
{
	...
	for (i=0; i<mesh_json.sta_num; i++) 
	{
		...
		sprintf(buffer, "%d", mesh_json.sta_info[i].rssi);
...
int send_home_conf_to_client(int fd)
{
	...
	#if 0
	for (i=0; i < mesh_json_s.child_num; i++)
	{
		APP_DEBUG("[zzk]mac = %s; device_name = %s \n",mesh_json_s.child_info[i].mac, mesh_json_s.child_info[i].device_name);
		reset_child_mesh_name(16, mesh_json_s.child_info[i].mac, mesh_json_s.child_info[i].device_name);
	}
	#endif
...
#if 0
{
	...
	if (wanRate.txRate < 1024)
		sprintf(buffer, "%.lfKBps", wanRate.txRate);
	else
		sprintf(buffer, "%.lfMBps", wanRate.txRate/1024);	
	cJSON_AddStringToObject(parameters, "wanUpSpeed", buffer);

	if (wanRate.rxRate < 1024)
		sprintf(buffer, "%.lfKBps", wanRate.rxRate);
	else
		sprintf(buffer, "%.lfMBps", wanRate.rxRate/1024);
	...
	sprintf(buffer, "%d", mesh_json_s.sta_num);
...
for (i=0; i<mesh_json_s.child_num; i++)
{
	...
	memset(buffer, 0, sizeof(buffer));
	//get_child_mesh_name(buffer, mesh_json_s.main_info.neighbor_num, mesh_json_s.main_info.neighbor_info[i].mac, mesh_json_s.main_info.neighbor_info[i].name);
	APP_DEBUG("[%s%d]child name = %s\n", __FUNCTION__, __LINE__, mesh_json_s.child_info[i].device_name);
	cJSON_AddStringToObject(arrayobj, "childName", mesh_json_s.child_info[i].device_name);
	sprintf(buffer, "%d", mesh_json_s.child_info[i].sta_num);
	cJSON_AddStringToObject(arrayobj, "childDevNum", buffer);	
	cJSON_AddStringToObject(arrayobj, "childDevMac", mesh_json_s.child_info[i].mac);	
}
	...
	*/
	if ( data_begin == NULL )
	{
		...
		else
		{
			...
			/*处理request请求*/
			app_request_flag = get_app_request_type(jason_data);
		}
	}
	else
	{
		data_length = strlen(data_begin);
		APP_DEBUG("data_length = %d\n", data_length);

	
		/*处理json设置*/
		memcpy(jason_data, data_begin, data_length);	
		//APP_DEBUG("[%s-%d]data_strlen = %d\n", __FUNCTION__, __LINE__, strlen(data_begin));
		APP_DEBUG("[%s-%d]jason_data = %s\n", __FUNCTION__, __LINE__, jason_data);
