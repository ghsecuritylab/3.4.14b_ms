...
void addExtStationInfoToTopology(cJSON * obj, RTK_LAN_DEVICE_INFO_T *devinfo)
{
    ...
    if(head != NULL)
    {
        ...
        {
        	sprintf(tmp, "%s", head->valuestring);
	...
	str_to_mac(tmp, child_mac);
	if (get_child_mesh_name(app_device_name, child_mac, "NULL") == 0)
	{
		cJSON_ReplaceItemInObject(obj, "device_name" , cJSON_CreateString(app_device_name));
		//printf("[%s]cJSON_ReplaceItemInObject app_device_name = %s\n", __FUNCTION__, app_device_name);
	}
    ...
    if(head != NULL)
    {
        ...
        for(pos = head->child; pos != NULL; pos = pos->next)
        {
		    ...
		    if(nb != NULL)
		    {
				...
				str_to_mac(tmp, child_mac);
				//printf("[%s]will get neighbor_mac %s name\n", __FUNCTION__, child_mac);
				if (get_child_mesh_name(app_device_name, child_mac, "NULL") == 0)
				{
...
int get_mesh_json_sta_info(P_MESH_DEV_INFO mesh_json)
{
	...
	{ 
		...
		for(i=0; i<size; i++)  
		{
			...
			APP_DEBUG( "mesh_name = %s\n", mesh_json->child_info[mesh_json->child_num].device_name);
			APP_DEBUG( "mesh_mac = %s\n", mesh_json->child_info[mesh_json->child_num].mac);
			#if 0
			memset(buffer, 0, sizeof(buffer));
			APP_DEBUG("mac = %s, name = %s\n",mesh_json->child_info[mesh_json->child_num].mac, device_name);
			ret = get_child_mesh_name(buffer,16, mesh_json->child_info[mesh_json->child_num].mac, device_name);
			//sprintf(cmd, "%s;%s", mesh_json->child_info[mesh_json->child_num].mac, buffer);
			//apmib_set(MIB_KLINK_SLAVE1_MESH_NAME+mesh_json->child_num, cmd);
			//printf(" set slave[%d] %s\n", mesh_json->child_num, cmd);
			sprintf(mesh_name, "%s", buffer);
			strcpy(mesh_json->child_info[mesh_json->child_num].device_name, mesh_name);
			#endif
			...
			/* 子节点邻居节点信息 */
			cJSON *arrayItem_1=cJSON_GetObjectItem(object, "neighbor_devices");  
			if(arrayItem_1!=NULL)  
			{  
				int nb_size=cJSON_GetArraySize(arrayItem_1);  
				int step = 0;
				//printf("main mesh have neighbor_devices number = %d.\n",size);  
				for(step=0; step<nb_size; step++)  
				{
					cJSON *object;
					int nb_num = mesh_json->main_info.neighbor_num;
					object=cJSON_GetArrayItem(arrayItem_1,step);  
					item = cJSON_GetObjectItem(object, "neighbor_mac"); 
					if(item != NULL)
					{
						DTRACE(DTRACE_APPSERVER,"neighbor_mac is %s\n", item->valuestring);
						...
						if (ret < 0)
							...
							strcpy(mesh_json->main_info.neighbor_info[nb_num].mac, buffer);
					}
					...
					if(item != NULL)
					{
						...
						strcpy(mesh_json->main_info.neighbor_info[nb_num].name, item->valuestring);
					}
		
					item=cJSON_GetObjectItem(object, "neighbor_rssi");	
					if(item != NULL)
					{
						strcpy(mesh_json->main_info.neighbor_info[nb_num].rssi, item->valuestring);
					}
					...
					{
						//printf("station_hostname is %s\n", item->valuestring);
						strcpy(mesh_json->main_info.neighbor_info[nb_num].band, item->valuestring);
					}
			...
			#if 1	//zzk not finish 
			/* 子节点的子节点 */
			cJSON * cc_item = cJSON_GetObjectItem(object, "child_devices");  
			if(cc_item!=NULL)  
			{
				int cc_size = cJSON_GetArraySize(cc_item); 
				int cc_i = 0;
				for (cc_i = 0; cc_i < cc_size; cc_i++)
				{
					cJSON *cc_object;
					char *cc_device_name = NULL;
					cc_object=cJSON_GetArrayItem(cc_item, cc_i);  

					item = cJSON_GetObjectItem(cc_object, "mac_address");	
					if(item != NULL)
					{
					...
					item = cJSON_GetObjectItem(cc_object, "ip_addr");	
					if(item != NULL)
					{
						//printf("ip_addr is %s\n", item->valuestring);
						strcpy(mesh_json->child_info[mesh_json->child_num].ip, item->valuestring);
					...
					item = cJSON_GetObjectItem(cc_object, "device_name");	
					if(item != NULL)
					{
						device_name = item->valuestring;
						APP_DEBUG("device_name is %s\n", device_name);
						strcpy(mesh_json->child_info[mesh_json->child_num].device_name, device_name);
					}
					else
					{
					...
					z = mesh_json->child_num;
					/* add station to child_info and neighbor */
					get_station_info_json(cc_object, device_name, mesh_json, &mesh_json->child_info[z], p_neighbor_x);
					mesh_json->child_num++;
					add_child_node_to_neighbor(mesh_json, &mesh_json->child_info[z]);
				}
			}
...
int add_child_info_into_mesh(P_MESH_DEV_INFO mesh_json, cJSON *pos)
{
	cJSON * item;
	char buffer[32] = {0};
	...
	item = cJSON_GetObjectItem(pos, "device_name");
	if(item != NULL)
	{
		strcpy(mesh_json->child_info[mesh_json->child_num].device_name, item->valuestring);
		strcpy(device_name, item->valuestring);
	}
	else
	{
		strcpy(device_name, "NULL");
	}

	item = cJSON_GetObjectItem(pos, "ip_addr"); 
	if(item != NULL)
	{
		//printf("ip_addr is %s\n", item->valuestring);
		strcpy(mesh_json->child_info[mesh_json->child_num].ip, item->valuestring);
	...
	item = cJSON_GetObjectItem(pos, "mac_address"); 
	if(item != NULL)
	{
		APP_DEBUG("mac_address is %s\n", item->valuestring);
		memset(buffer, 0, sizeof(buffer));
		ret = str_to_mac(item->valuestring, buffer);
		if (ret < 0)
		{
	...
	idx = mesh_get_neighbor_id(mesh_json, &mesh_json->child_info[mesh_json->child_num].mac);
	P_MESH_NEIGHBOR_INFO p_neighbor_x = &mesh_json->main_info.neighbor_info[idx]; 

	z = mesh_json->child_num;
	/* add station to child_info and neighbor */
	get_station_info_json(pos, device_name, mesh_json, &mesh_json->child_info[z], p_neighbor_x);
	mesh_json->child_num++;

	add_child_node_to_neighbor(mesh_json, &mesh_json->child_info[z]);
	
	/* 子节点邻居节点信息 */
	cJSON *arrayItem_1=cJSON_GetObjectItem(pos, "neighbor_devices");  
	if(arrayItem_1!=NULL)  
	{  
		int nb_size=cJSON_GetArraySize(arrayItem_1);  
		int step = 0;
	...
	arrayItem=cJSON_GetObjectItem(pos, "child_devices");  
	if(arrayItem!=NULL)  
	{
		/* 回调 */
		for(pos_c = arrayItem->child; pos_c != NULL; pos_c = pos_c->next)
		{		
			add_child_info_into_mesh(mesh_json, pos_c);
		}
	}
	return 0;
}
...
int get_mesh_json_sta_info(P_MESH_DEV_INFO mesh_json)
{
	...
	{
		/* 回调 */
		for(pos = arrayItem->child; pos != NULL; pos = pos->next)
		{		
			add_child_info_into_mesh(mesh_json, pos);
		}
		/* end */		
	}
...
/**/
int get_lan_dev_linktime(P_MESH_CHILD_INFO         dst_childx, RTK_LAN_DEVICE_INFO_Tp devinfo, int num)
{
	int i = 0;
	int j = 0;
	char mac_b[32] = {0};
	...
	for(i=0; i<dst_childx->sta_num; i++)
	{
		for(j=0; j<num; j++)
		{
			...
			sprintf(mac_b, "%02X:%02X:%02X:%02X:%02X:%02X",
				devinfo[j].mac[0],devinfo[j].mac[1],devinfo[j].mac[2],devinfo[j].mac[3],devinfo[j].mac[4],devinfo[j].mac[5]);
			if (!strcasecmp(dst_childx->sta_info[i].mac, mac_b) && (devinfo[j].linktime > 0))
			{
				pTM = localtime(&devinfo[j].linktime);
				sprintf(time_b, "%04d-%02d-%02d %02d:%02d", pTM->tm_year+1900, pTM->tm_mon+1, pTM->tm_mday, pTM->tm_hour, pTM->tm_min);
				strcpy(dst_childx->sta_info[i].link_time, time_b);
			}
...
int send_child_json(int fd, P_MESH_DEV_INFO mesh_json, char *pchildx_mac)
{
	...
	else
	{
		...
		get_lan_dev_linktime(&dst_childx, devinfo, num);
		...
		/* >-75dBm good，>-83 & <-75dBm better, <-83dBm poor */
		if (atoi(dst_childx.rssi) > 25)
			cJSON_AddStringToObject(parameters, "linkQuality", "good");
		else if (atoi(dst_childx.rssi) > 17 && atoi(dst_childx.rssi) < 25)
			cJSON_AddStringToObject(parameters, "linkQuality", "better");
		else 
			cJSON_AddStringToObject(parameters, "linkQuality", "poor");
		#else
		/* >-82dBm good，>-83 & <-75dBm better, <-83dBm poor */
		APP_DEBUG("rssi = %s\n", dst_childx.rssi);
		if (atoi(dst_childx.rssi) > 18)
			cJSON_AddStringToObject(parameters, "linkQuality", "good");
		else 
			cJSON_AddStringToObject(parameters, "linkQuality", "poor");
		#endif
		
		char targetInfo[32] = {0};
		memset(buffer, 0, sizeof(buffer));
		str_to_mac_addr(buffer, dst_childx.mac);

		APP_DEBUG("[getSlaveVersion] sn mac = %s\n", buffer);
		getSlaveVersion(targetInfo, buffer, 1);
		APP_DEBUG("[getSlaveVersion] sn  = %s\n", targetInfo);
		cJSON_AddStringToObject(parameters, "sn", targetInfo);

		//memset(buffer, 0, sizeof(buffer));
		//get_child_mesh_name(buffer, mesh_json->child_num, dst_childx.mac, dst_childx.device_name);
		//printf("!!! location = %s\n", buffer);
...
int get_slave_ip_from_device_name(const char* dev_name, char * p_dev_ip)
{
	int i = 0;
	...
	for (i=0; i < mesh_json.child_num; i++)
	{
		printf("mesh_json.child_info[i].device_name = %s\n",mesh_json.child_info[i].device_name);
		printf("mesh_json.child_info[i].ip = %s\n",mesh_json.child_info[i].ip);
		printf("mesh_json.child_info[i].mac = %s\n",mesh_json.child_info[i].mac);
		if (!strcasecmp(mesh_json.child_info[i].device_name, dev_name))
		{
			strcpy(p_dev_ip, mesh_json.child_info[i].ip);
			return 1;
		}
...
int reset_child_mesh_name(  char *p_mac, char *p_name)
{
	int i = 0;
	char value[512] = {0};
	char MAC[32] = {0};
	...
	/* set main mesh's device name */
	if ( !strcasecmp(p_mac, wanAdrr) )
	{
		apmib_set(MIB_MAP_DEVICE_NAME, p_name);
		//apmib_update_web(CURRENT_SETTING);
		return 0;
	}
	...
	for (i=0; i < 16; i++)
	{
		...
		APP_DEBUG("[get_child_mesh_name] flash get MIB_KLINK_SLAVE1_MESH_NAME+%d value = %s\n", i,value);
		...
		if ( (NULL != p1) )
		{
			...
			APP_DEBUG("reset mac = %s name = %s\n", v_mac, p_name);
			sprintf(value, "%s;%s", MAC, p_name);
			apmib_set(MIB_KLINK_SLAVE1_MESH_NAME+i, value);
			APP_DEBUG("apmib MIB_KLINK_SLAVE1_MESH_NAME+%d = %s\n", i, value);
			return 0;
		}
		else if (NULL != p2)
		{
	...
	sprintf(value, "%s;%s", MAC, p_name);
	apmib_set(MIB_KLINK_SLAVE1_MESH_NAME+vilid_id, value);
	APP_DEBUG("[reset_child_mesh_name] apmib_set(MIB_KLINK_SLAVE1_MESH_NAME+%d = %s\n", vilid_id, value);
	//apmib_update_web(CURRENT_SETTING);
...
*/
int get_child_mesh_name(char *p_buffer, char *p_mac, char *p_name)
{
	int i = 0;
	char value[512] = {0};
	char MAC[32] = {0};
	...
	if (p_mac == NULL || strlen(p_mac) == 0)
	{
		sprintf(p_buffer, "%s", p_name);
		return -1;
	}
	sprintf(MAC, "%s", p_mac);

	APP_DEBUG("[get_child_mesh_name] flash get mac [%s]'s name\n", p_mac);
	...
	for (i=0; i < 16; i++)
	{
		...
		{
			sscanf(value, "%[^;];%[^;]", v_mac, v_name);
			APP_DEBUG("p2 v_mac = %s name = %s\n", v_mac, v_name);
			sprintf(p_buffer, "%s", v_name);
			return 0;
		}
	}

	APP_DEBUG("[get_child_mesh_name] cannot find mac's name\n");
	sprintf(p_buffer, "%s", p_name);
	
	return 1;
}

int set_child_mesh_name(char *p_mac, char *p_name)
{
	int i = 0;
	int num = 0;
	char value[512] = {0};
	...
	if (p_mac == NULL || strlen(p_mac) == 0 || strlen(p_name) == 0)
	{
		return -1;
	}
	sprintf(MAC, "%s", p_mac);
	...
	{
		apmib_get(MIB_KLINK_SLAVE1_MESH_NAME+i, (void *)value);
		APP_DEBUG("[Set_child_mesh_name] flash get  MIB_KLINK_SLAVE1_MESH_NAME+%d = %s\n",i, value);
		to_upper(MAC);
		p1 = strstr(value, p_mac);
		p2 = strstr(value, MAC);
		...
		{
			sprintf(value, "%s;%s", MAC, p_name);
			APP_DEBUG("[Set_child_mesh_name] flash set  MIB_KLINK_SLAVE1_MESH_NAME+%d = %s\n",i, value);
			apmib_set(MIB_KLINK_SLAVE1_MESH_NAME+i, value);
			//apmib_update_web(CURRENT_SETTING);
			return 0;
		}
	}
	APP_DEBUG("cannot find mac !!! \n");
	return 0;
}

int str_to_mac_addr(char *buf, char *mac)
{
	char *p = NULL;
	char *p_tmp = mac;
