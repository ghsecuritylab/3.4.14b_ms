...
//#ifdef CONFIG_RTL865X_AC /* In order to accept empty character in HostName. Keith */	
	...
	int first_flag=0;
#endif
    /*haopeng add for kingsignal option*/
	char oui[VENDOR_BUFSIZE];						  
	char serial_number[VENDOR_BUFSIZE];	   
	char product_class[VENDOR_BUFSIZE];	   
	uint32_t oui_size = VENDOR_BUFSIZE;
	uint32_t serial_number_size =VENDOR_BUFSIZE;
	uint32_t product_class_size = VENDOR_BUFSIZE;
	/*haopeng add end*/
...
system("route del default dev ppp0 > /dev/null 2>&1");
	...
	if (detect) {
		...
		if (dhcpc_retry >= 2) {
          ...
          /*haopeng 2019-12-10:if get kingsignal vendor in offer packet,set flag to tmp directory*/
			get_vendor_specific_option(&packet,
					 DEVICE_OUI_CODE, (uint8_t *)oui, &oui_size,
					 DEVICE_SERIAL_NUMBER_CODE, (uint8_t *)serial_number, &serial_number_size,
					 DEVICE_PRODUCT_CLASS_CODE, (uint8_t *)product_class, &product_class_size);
	/* 
			printf("***oui[length: %u]: [%s]\n"
			 "***serial_number[length: %u]: [%s]\n"
			"***productClass[length: %u]: [%s]\n",
			oui_size, oui,
			serial_number_size, serial_number,
			product_class_size, product_class);
        */
		if (get_option(&packet, DHCP_KINGSIGNAL_OPTS) != NULL)  
		{
			if((!strcmp("kingsignal",oui))&&(!strcmp("d05157",serial_number))&&(!strcmp("mesh",product_class)))
			{
             system("echo 1 > /tmp/netFlag");
			}
		}
