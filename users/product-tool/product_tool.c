...
#define ROUTER_SET_FLG_CALIBRATED  5
...
int FactorySet(char *mac)
{
    ...
    wlan_idx  = 0;
	vwlan_idx = 0;
    if (SetDefAndCurrApmib(MIB_WLAN_CHANNEL, (void *)&mibVal) <= 0)
	{
		DPrintf("set MIB_WLAN_CHANNEL err.\n");  
        wlan_idx = old_wlan_idx;
        vwlan_idx = old_vwlan_idx;
        return 0;
	}
...
int IsArrayAllZero(char *arr, int len)
{
    int i;
...
int IsCalibrated()
{
    int old_wlan_idx;
    int old_vwlan_idx;
    char pwrlevel5GHT40_1S_A[MAX_5G_CHANNEL_NUM_MIB]={0};
    ...
    wlan_idx  = 0;
	vwlan_idx = 0;
    apmib_get(MIB_HW_TX_POWER_5G_HT40_1S_A, pwrlevel5GHT40_1S_A);
    if(IsArrayAllZero(pwrlevel5GHT40_1S_A, sizeof(pwrlevel5GHT40_1S_A)))
    {
        wlan_idx = old_wlan_idx;
        vwlan_idx = old_vwlan_idx;
        return 0;
		    ...
		    if(argc >= 2)
		    {
				if (!strcmp(argv[1], "set" ))
				{
                    if(argc >= 4)
                    {
                        ...
                        else if(!strcmp(argv[2], "pairing_flg"))
    					{
    						DTRACE("set pairing_flg %s\n", argv[3]);
                            ...
                            if(strlen(argv[3]) == 1)
                            {
                                ...
                                {
                                    SetMibSetFlg(ROUTER_SET_FLG_PAIRING, 0);
                                }
                                strcpy(send_buf,"ROUTER pass");
                            }
                            else
                ...
                else if(!strcmp(argv[1], "detect"))
				{
                    ...
                    if(IsCalibrated())
                    {
                        flg = (flg | (1<<ROUTER_SET_FLG_CALIBRATED));
                    }
                    else
                    {
                        flg = (flg & (~(1<<ROUTER_SET_FLG_CALIBRATED)));
                ...
                else if(!strcmp(argv[1], "default-all"))
				{
                    DTRACE("product_tool default-all\n");
                    
                    sprintf(cmd_buf, "flash default");
                    system(cmd_buf);
					strcpy(send_buf,"ROUTER pass");
