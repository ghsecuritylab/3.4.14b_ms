...
int isInParentContrl(char *p_filter_mac);
...
int setMACFilter(void)
{
	...
	for (index=1; index<=entryNum; index++) {
		...
		*((char *)&entry) = (char)index;
		...
		if (1 == isInParentContrl(macEntry))
		{
			...
			RunSystemCmd(NULL_FILE, Iptables, DEL, FORWARD, match, "mac" ,mac_src, macEntry, jump, ACCEPT, NULL_STR);
			RunSystemCmd(NULL_FILE, Iptables, DEL, INPUT, match, "mac" ,mac_src, macEntry, jump, ACCEPT, NULL_STR);
		}
...
/*
	return 1:need set 
*/
int isInMacFilter(char *p_filter_mac)
{
	char macEntry[30];
	int entryNum=0, index;
	int entryEnable = 0;
	...
	if (NULL == p_filter_mac || strlen(p_filter_mac) == 0)
		return -1;
	...
	for (index=1; index<=entryNum; index++) {
		...
		*((char *)&entry) = (char)index;
		apmib_get(MIB_MACFILTER_TBL, (void *)&entry);
		sprintf(macEntry,"%02X:%02X:%02X:%02X:%02X:%02X", entry.macAddr[0], entry.macAddr[1], entry.macAddr[2], entry.macAddr[3], entry.macAddr[4], entry.macAddr[5]);
		if (0 == strcasecmp(p_filter_mac, macEntry))
		{
			printf("find it !!!\n");
			return 1;
		}
...
int isInParentContrl(char *p_filter_mac)
{
	int j = 0;
	int i = 0;
	MACFILTER_T macEntrytmp;
	int parentEntryNum = 0;
	PARENT_CONTRL_T entry;

	if (strlen(p_filter_mac)==0)
		return 0;
	...
	for (i=1; i<=parentEntryNum; i++) 
	{
		...
		*((char *)&entry) = (char)i;
		...
		{
			if (!strcasecmp(dev_info.mac[j], p_filter_mac))
			{
				return 1;
			}
		}
...
#else 
int parentControlSetting(int parentContrlAction,const char* macEntry)
{
	char cmdBuffer[80];
	int num = 0;
	int i=0;
	...
	if (macEntry == NULL || strlen(macEntry) == 0)
		return -1;
	if (6 != sscanf(macEntry, "%02X:%02X:%02X:%02X:%02X:%02X", &addr[0], &addr[1], &addr[2], &addr[3],&addr[4],&addr[5]))
		return -1;

	if (1 == isInMacFilter(macEntry))
	{
		printf("It[%s] is in mac filter !!!\n", macEntry);
		return 0;
	}
