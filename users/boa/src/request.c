...
extern int add_user(request *wp);
...
//	}//end if(check_auth_flag == 0 && req->auth_flag == 0)
    ...
    if(!strstr(req->request_uri, WEB_PAGE_WIZARD)
        && !strstr(req->request_uri, WEB_PAGE_COUNTDOWN )
        && !strstr(req->request_uri, "/boafrm/formUpgradeSlave" )
        && !strstr(req->request_uri, "/boafrm/formRemoteUpgrade" ))
    {
         if(req->request_uri && req->header_referer)
        {
        ...
        if(req->request_uri && req->header_referer)
       {
           if(strstr(req->header_referer, WEB_PAGE_COUNTDOWN))
           {
               access_flg = 1;
           }
       }
        ...
        if(map_state == 2)
        {
            if(mib_val != 1 && access_flg == 0 && map_configured_band != 3)
            {
    ...
    if((is_valid_user(req) != 1) 
        ...
        && (!strstr(req->request_uri, WEB_PAGE_WIZARD))
        && (!strstr(req->request_uri, WEB_PAGE_COUNTDOWN))) //not auth and not login page or wizard page
    {
        need_auth_flg = 1;
        ...
        if(req->request_uri && req->header_referer)
        {
            ...
            if(strstr(req->header_referer, WEB_PAGE_COUNTDOWN)) //request frome COUNTDOWN page,no need auth
            {
                need_auth_flg = 0;
            }
