...
int RecvData(int sock_fd, char *buf, int len, int time_out_sec)
{
    ...
    while(1)
    {
        ...
        if(retval == -1 && errno == EINTR)
        {
            ...
            if(time_out_sec <= 0)
            {
                DTRACE(DTRACE_SLAVE_UPGRADE, "err, retval = %d, errno = %s.\n", retval, strerror(errno));
    		    return -1;
            }
            continue;
        }
...
int SendFileToSlave(char *file_data, int file_len, char *slave_ip, int slave_port)
{
    ...
    time_out.tv_sec = 8;
    time_out.tv_usec = 0;
    setsockopt(sock_fd,SOL_SOCKET,SO_SNDTIMEO,(char *)&time_out, sizeof(struct timeval)); 
    ...
    if(RecvData(sock_fd, buf, sizeof(buf), 30) > 0)
    {
...
int SlaveStartRemoteUpgrade(char *slave_ip, int slave_port)
{
    ...
    memset(buf, 0, sizeof(buf));
    int ret_val = RecvData(sock_fd, buf, sizeof(buf), 93);
    if(ret_val > 0)
    {        
        close(sock_fd); 
        DTRACE(DTRACE_SLAVE_UPGRADE, "%s \n", buf);
