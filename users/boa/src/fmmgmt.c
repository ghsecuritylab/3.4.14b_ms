...
//dhcp list
	...
	for(i=0;i<dhcp_device_num;i++)
	{
        ...
        if(devInfoIdx < MAX_STA_NUM)
		{
    		...
    		pdevinfo[devInfoIdx].dhcp_client_flg = 1;
        }
	}
...
static int macToHex(char *buf, char *mac)
{
	char *p = NULL;
	char *p_tmp = mac;
...
extern void getSlaveVersion(char targetVersion[], char* mac, int dataType);
...
void formRemoteUpgradeSlave(request *wp, char *path, char *query)
{
    ...
    if(ip_addr[0])
    {
        ...
        else
        {
            ...
            {
                sprintf(tmp_buf, "download fail!please check your network.");
            }
        }
    }
    else
