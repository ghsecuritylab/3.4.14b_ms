...
int dhcpClientList(request *wp, int argc, char **argv)
{
    ...
    struct in_addr br0_ip;
    struct in_addr subnet_mask;
    struct in_addr curr_ip;
    GetInAddr("br0", IP_ADDR, (void *)&br0_ip );
    GetInAddr("br0", SUBNET_MASK, (void *)&subnet_mask );
	...
	while (1) {
        ...
        curr_ip.s_addr = inet_addr(ipAddr);
        if((br0_ip.s_addr&subnet_mask.s_addr) != (curr_ip.s_addr&subnet_mask.s_addr))
        {
            continue;
        }
